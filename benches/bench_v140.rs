...
fn setup_join_tables(engine: &mut ExecutionEngine<MemoryStorage>, left_rows: usize, right_rows: usize) {
    ...
    for i in 0..left_rows {
        engine
            .execute(&format!("INSERT INTO t1 VALUES ({}, {})", i, i % 50))
            .unwrap();
    }

    for i in 0..right_rows {
        engine
            .execute(&format!("INSERT INTO t2 VALUES ({}, 'name_{}')", i, i))
            .unwrap();
    }
}
...
fn bench_tablescan_full(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("full_scan", size), &size, |b, _| {
            b.iter(|| engine.execute("SELECT * FROM t1").unwrap());
        });
    }
...
fn bench_filter_and(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("and", size), &size, |b, _| {
            b.iter(|| {
                engine
                    .execute("SELECT * FROM t1 WHERE id > 10 AND value < 50")
                    .unwrap()
            });
        });
    }
...
fn bench_aggregate_group_by(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("group_by", size), &size, |b, _| {
            b.iter(|| {
                engine
                    .execute("SELECT value, COUNT(*) FROM t1 GROUP BY value")
                    .unwrap()
            });
        });
    }
