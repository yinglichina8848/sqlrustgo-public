#!/bin/bash
# Sysbench OLTP Insert Test for SQLRustGo
# Usage: ./oltp_insert.sh [threads] [time]
#
# Per G12: oltp_insert 基准 (批量插入)
...
PORT=${PORT:-3306}
USER=${USER:-root}
PASSWORD=*** +%Y%m%d_%H%M%S)_oltp_insert"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "SQLRustGo Sysbench OLTP Insert"
echo "=========================================="
...
OUTPUT_FILE="$RESULTS_DIR/oltp_insert_t${THREADS}.log"
sysbench oltp_insert \
  --db-driver=mysql \
  --mysql-host=$HOST --mysql-port=$PORT \
  --mysql-user=$USER --mysql-password=$PASSWORD \
