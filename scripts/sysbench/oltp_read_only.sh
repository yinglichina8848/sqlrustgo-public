#!/bin/bash
# Sysbench OLTP Read Only Test for SQLRustGo
# Usage: ./oltp_read_only.sh [threads] [time]
#
# Per G12: oltp_read_only 基准
...
PORT=${PORT:-3306}
USER=${USER:-root}
PASSWORD=${PASSWORD:-sqlrustgo}
DB=${DB:-sbtest}
TABLE_SIZE=${TABLE_SIZE:-10000}

RESULTS_DIR="./sysbench_results_$(date +%Y%m%d_%H%M%S)_oltp_read_only"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "SQLRustGo Sysbench OLTP Read Only"
echo "=========================================="
...
OUTPUT_FILE="$RESULTS_DIR/oltp_read_only_t${THREADS}.log"
sysbench oltp_read_only \
  --db-driver=mysql \
  --mysql-host=$HOST --mysql-port=$PORT \
  --mysql-user=$USER --mysql-password=$PASSWORD \
