...
# Usage: ./oltp_point_select.sh [threads] [time]
#
# Per G12: oltp_point_select 基准
...
RESULTS_DIR="./sysbench_results_$(date +%Y%m%d_%H%M%S)_oltp_point_select"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "SQLRustGo Sysbench OLTP Point Select"
echo "=========================================="
...
OUTPUT_FILE="$RESULTS_DIR/oltp_point_select_t${THREADS}.log"
sysbench oltp_point_select \
  --db-driver=mysql \
  --mysql-host=$HOST --mysql-port=$PORT \
  --mysql-user=$USER --mysql-password=$PASSWORD \
...
echo "  (parse: cat $OUTPUT_FILE | grep -E 'transactions|queries per sec|95th|99th')"
