#!/usr/bin/env python3
"""MySQL Ladder Soak Test — ladder: 2h → 4h → 8h → 16h → 24h → 48h → 72h
Server stays up across all runs. Persistent connections, 8 threads.
Post Gitea issue #3225 after each run.
"""
...
# (name, dur_min, max_q_per_thread) — max_q HIGH so wall-clock deadline is the constraint
DURATIONS = [
    ("2h",  120,  999_999_999),
    ("4h",  240,  999_999_999),
    ("8h",  480,  999_999_999),
    ("16h", 960,  999_999_999),
    ("24h", 1440, 999_999_999),
    ("48h", 2880, 999_999_999),
    ("72h", 4320, 999_999_999),
]
THREADS = 8
...
class MySQLClient:
    def __init__(self, host, port, user="root"):
        ...
        self.sock.settimeout(30)   # blocking with timeout
        self.seq = 0
        pkt = self._recv(4)
        self.seq = pkt[3]
        ...
        payload += bytes([45])  # utf8mb4_general_ci
        payload += b"\x00" * 23
...
def post_gitea(body):
    ...
    try:
        ...
        print(f"  [Gitea post error: {e}]"); return False

# ── Soak run ─────────────────────────────────────────────────────────────────────

server_proc = None

def run_soak(label, port, threads, dur_min, max_q_per_thread):
    ...
    def monitor():
        while not stop_monitor[0]:
            ...
            if server_proc and server_proc.poll() is None:
                rss = get_rss(server_proc.pid)
                fd  = get_fd(server_proc.pid)
        ...
        try:
            ...
            while time.time() < deadline:
                if c.query(f"SELECT {cnt} AS n"):
                    ...
                    err += 1
                cnt += threads
                if q >= max_q_per_thread:
                    break
            c.close()
        except Exception:
            err += 1
        total_q[0] += q; total_e[0] += err
    ...
    stop_monitor[0] = True; monitor_t.join(timeout=2)
    actual = time.time() - t0_wall
    q = total_q[0]; e = total_e[0]; qps = q / actual if actual > 0 else 0
    rss_growth = (rss_samples[-1][1] - rss_samples[0][1]) if len(rss_samples) > 1 else 0
...
def main():
    ...
    wait_server(port)
    rss_start = get_rss(server_proc.pid)
    fd_start  = get_fd(server_proc.pid)
    print(f"Server PID={server_proc.pid} port={port} RSS={rss_start}KB FD={fd_start}")
...
"""
    ...
    print("\nAll runs complete — shutting down server")
...
"""
    ...
    md += f"""
**Total: {total_q:,} queries, {total_e} errors, {sum(1 for r in results if r['passed'])}/{len(results)} runs passed**
...
"""
    ...
    with open(report_path, "w") as f:
        f.write(md)
    print(f"\nReport: {report_path}")
    post_gitea(f"## 🏁 All {len(results)} Soak Runs Complete — {sum(1 for r in results if r['passed'])}/{len(results)} PASSED\n\nTotal: {total_q:,} queries, {total_e} errors\n\nReport: `{report_path}`")
