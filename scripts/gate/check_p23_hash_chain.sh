...
#
# Verifies:
# 0. Inline oracle (V4 fix): cargo test --test oracle_p23_hash_chain
...
ORACLE_OUTPUT=$(cargo test --test oracle_p23_hash_chain --all-features 2>&1)
ORACLE_EXIT=$?
if [ $ORACLE_EXIT -eq 0 ]; then
    ORACLE_PASSED=$(echo "$ORACLE_OUTPUT" | grep -E "test result.*ok" | head -1)
    echo "  [0/8] PASS: oracle_p23_hash_chain $ORACLE_PASSED"
else
    echo "  [0/8] FAIL: oracle_p23_hash_chain (exit=$ORACLE_EXIT)"
...
echo "  [1/8] ✅ PASS: tests/hash_chain_harness.rs present"
...
echo "  [2/8] ✅ PASS: tests/hash_chain_test.rs present + registered"
...
echo "  [3/8] ✅ PASS: 4 categories covered (total: $N_TESTS tests, ≥20)"

# 4. cargo check
if cargo check --test hash_chain_test 2>&1 | tail -3 | grep -q "Finished\|Compiling"; then
    echo "  [4/8] ✅ PASS: hash_chain_test compiles"
else
...
if cargo check -p sqlrustgo-gmp 2>&1 | tail -3 | grep -q "Finished\|Compiling"; then
    ...
    if cargo check -p sqlrustgo-gmp 2>&1 | grep -q "error\["; then
        ...
        echo "  [6/8] ✅ PASS: crates/gmp compiles (no regression)"
    fi
fi
...
if echo "$LEN_OUT" | grep -q ':016x'; then
    echo "  [7/8] ✅ PASS: link() output is 16 hex chars (64-bit simplified SHA-256, hex-only)"
else
