...
#
# Verifies:
# 0. Inline oracle (V4 fix): cargo test --test oracle_p34_parallel_executor
...
ORACLE_OUTPUT=$(cargo test --test oracle_p34_parallel_executor --all-features 2>&1)
ORACLE_EXIT=$?
if [ $ORACLE_EXIT -eq 0 ]; then
    ORACLE_PASSED=$(echo "$ORACLE_OUTPUT" | grep -E "test result.*ok" | head -1)
    echo "  [0/8] PASS: oracle_p34_parallel_executor $ORACLE_PASSED"
else
    echo "  [0/8] FAIL: oracle_p34_parallel_executor (exit=$ORACLE_EXIT)"
...
echo "  [1/8] ✅ PASS: tests/parallel_executor_harness.rs present"
...
echo "  [2/8] ✅ PASS: tests/parallel_executor_test.rs present + registered"
...
if cargo check --test parallel_executor_test 2>&1 | tail -3 | grep -q "Finished\|Compiling"; then
    echo "  [4/8] ✅ PASS: parallel_executor_test compiles"
else
...
# 5. ≥20 tests pass
RAW=$(cargo test --test parallel_executor_test 2>&1)
TEST_EXIT=$?
PASSED=$(echo "$RAW" | grep -E "test result.*ok" | grep -oE "[0-9]+ passed" | head -1)
if [ -z "$PASSED" ] && [ "$TEST_EXIT" -ne 0 ]; then
    echo "  ❌ FAIL: parallel_executor_test exited with $TEST_EXIT"
    echo "$RAW" | tail -5
    exit 1
fi
if [ -z "$PASSED" ]; then
...
echo "  [5/8] ✅ PASS: parallel_executor_test $PASSED (≥20)"
...
if cargo check -p sqlrustgo-executor 2>&1 | tail -3 | grep -q "Finished\|Compiling"; then
    echo "  [6/8] ✅ PASS: crates/executor (parallel_vector) compiles (no regression)"
else
    if cargo check -p sqlrustgo-executor 2>&1 | grep -q "error\["; then
        echo "  ❌ FAIL: crates/executor has compile errors"
        EX_OK=false
    else
        echo "  [6/8] ✅ PASS: crates/executor compiles (no regression)"
    fi
fi
[ "$EX_OK" = "true" ] || exit 1
