...
- Engine-side evaluation of the expression on INSERT/UPDATE.
- Engine-side materialisation of STORED columns.
- Engine-side computation of VIRTUAL columns on read.
- `GENERATED BY DEFAULT AS (expr)` variant.
- `GENERATED ALWAYS AS IDENTITY` integration with auto-increment.