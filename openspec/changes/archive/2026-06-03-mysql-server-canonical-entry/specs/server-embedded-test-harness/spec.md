# server-embedded-test-harness

## Purpose

`sqlrustgo_mysql_server::testing::start_ephemeral` is the
canonical entry point for the integration-test surface. It
boots an in-process MySQL server on an OS-assigned port and
returns a handle whose `Drop` cleans up the background thread
and the temporary data directory.
