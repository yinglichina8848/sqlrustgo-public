...
> **真实性边界**: README 只陈述已有文档或实测证据支持的状态；未完成项标为 `PARTIAL`、`DEFERRED` 或 `OPEN`。
...
| TPC-H SF=10 | harness 和 stub 证据存在，真实 SF=10 未完成 | [#4018 evidence](docs/releases/v3.12.0/evidence/issue-4018/4018_evidence.md) 明确 real SF=10 dbgen fixture blocked |
| Bulk-load SF=10 | OPEN / 未完成 | [#4020 evidence](docs/releases/v3.12.0/evidence/issue-4020/4020_evidence.md) 记录 schema creation 失败；不能当作完成 |
| MySQL wire + LOAD DATA hardening | PARTIAL | [V312-13 报告](docs/releases/v3.12.0/evidence/wire_load_data/V312-13-REPORT.md) failed_steps=0，但 SF=10 full bulk-load 不在该证据范围 |
| Sysbench | read_only baseline 已捕获；write/read_write 未完成 | [#4019 evidence](docs/releases/v3.12.0/evidence/issue-4019/4019_evidence.md) |
...
| SQLLogicTest | runner/gate 激活，仍有 v3.13 deferred 文件 | [v3.12 Scope Table](docs/releases/v3.12.0/SCOPE_TABLE_v3.12.md) |
...
| 完整 MySQL 5.7 替代 | PARTIAL | OPEN | 需要 SQLLogicTest、TPC-H correctness、wire、LOAD DATA、recovery、upgrade 等全部闭环 |
...
| v3.12.0 SF=1 close-out | PARTIAL | row count baseline 和 zero-row explanation 已形成 | cross-engine SHA256 和 zero-row correctness 仍 deferred |
| v3.12.0 SF=10 harness | PARTIAL | SQLite harness 可运行，stub fixture 上 22/22 captured | 不能宣称真实 SF=10 60M lineitem 已完成，也不能宣称 sqlrustgo-vs-sqlite parity |
| v3.12.0 Bulk-load SF=10 | OPEN | runner/gate/evidence 记录了失败原因 | 不能宣称 8 表真实 bulk-load 完成 |
...
- 当前 22/22 capture 使用的是 stub fixture。
- `dbgen` 不在该主机上，真实 SF=10 fixture 未生成。
- 还没有 sqlrustgo side-car 与 SQLite 的 SF=10 row-count/SHA256 parity 结果。

[#4020 evidence](docs/releases/v3.12.0/evidence/issue-4020/4020_evidence.md) 明确说明：

- Bulk-load runner 存在。
- 真实执行在 schema creation 阶段失败，脚本 exit=1。
- 没有 `bulk_load_summary.json`，没有 8 表导入完成证据。
...
| GMP compliance audit controls | PARTIAL / 持续补强 | [GMP 合规矩阵](docs/releases/v3.12.0/GMP_COMPLIANCE_MATRIX.md)、[V312-08](docs/releases/v3.12.0/v312-08-compliance-audit-report.md) |
