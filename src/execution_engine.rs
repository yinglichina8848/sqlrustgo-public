...
impl<S: StorageEngine + 'static> ExecutionEngine<S> {
    ...
    fn execute_drop_index(&self, idx: &DropIndexStatement) -> SqlResult<ExecutorResult> {
        ...
        //
        // (Cherry-pick resolution: HEAD side chosen over d4f6941b4 side —
        // HEAD uses a single `storage.write()` and proper IF EXISTS error
        // branch; d4f6941b4 leaks a read/write race and lacks the IF
        // EXISTS branch.)
        let mut storage = self.storage.write();
        let owner = storage
            .list_all_indexes()
