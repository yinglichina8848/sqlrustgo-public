...
impl TransactionManager {
    ...
    pub fn begin(&self) -> Result<u64, String> {
        let tx_id = {
            let mut next = self.next_tx_id.lock().expect("Failed to acquire next_tx_id lock");
            let id = *next;
            *next += 1;
            id
        };
        ...
        let snapshot: Vec<u64> = {
            let active = self.active_transactions.lock().expect("Failed to acquire active_transactions lock");
        ...
        self.active_transactions.lock().expect("Failed to acquire active_transactions lock").insert(tx_id, tx);
    ...
    pub fn commit(&self, tx_id: u64) -> Result<(), String> {
        let mut active = self.active_transactions.lock().expect("Failed to acquire active_transactions lock");
