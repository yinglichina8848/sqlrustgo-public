...
fn main() {
    ...
    // Count lineitem
    let cnt = e.execute("SELECT COUNT(*) FROM lineitem").unwrap();
    eprintln!("lineitem count: {}", cnt.rows.len());

    // Just run exists subquery alone (smaller test)
    let q_test = "SELECT COUNT(*) FROM lineitem l1 WHERE EXISTS (SELECT * FROM lineitem l2 WHERE l2.l_orderkey = l1.l_orderkey AND l2.l_suppkey <> l1.l_suppkey) LIMIT 100";
    let t0 = Instant::now();
    let r = e.execute(q_test).expect("test");
    eprintln!(
        "Test1 (EXISTS, 60K l1): rc={} in {:?}",
        r.rows.len(),
        t0.elapsed()
    );

    let q_test2 = "SELECT COUNT(*) FROM lineitem l1 WHERE NOT EXISTS (SELECT * FROM lineitem l3 WHERE l3.l_orderkey = l1.l_orderkey AND l3.l_suppkey <> l1.l_suppkey AND l3.l_receiptdate > l3.l_commitdate)";
    let t0 = Instant::now();
    let r = e.execute(q_test2).expect("test");
    eprintln!(
        "Test2 (NOT EXISTS, 60K l1): rc={} in {:?}",
        r.rows.len(),
        t0.elapsed()
    );
}
