...
const OLAP: &[(&str, &str)] = &[
    ...
    (
        "Q15",
        "SELECT s_suppkey, s_name FROM supplier, lineitem \
...
fn oltp_insert_orders(rng: &mut ThreadRng) -> String {
    ...
    let priority =
        ["1-URGENT", "2-HIGH", "3-MEDIUM", "4-LOW", "5-NOT SPECIFIED"][rng.gen_range(0..5)];
...
fn pick_oltp_op(rng: &mut ThreadRng) -> (&'static str, fn(&mut ThreadRng) -> String) {
    ...
    match rng.gen_range(0u32..16) {
        ...
        1 => (
            "insert",
            oltp_insert_lineitem as fn(&mut ThreadRng) -> String,
        ),
...
fn count_table(conn: &mut MySqlConnection, tbl: &str) -> u64 {
    ...
    match conn.execute(&sql) {
        Ok(ResultSet::Select { rows, .. }) if !rows.is_empty() => rows[0][0].parse().unwrap_or(0),
        _ => 0,
    }
}
...
fn parse_args() -> Args {
    ...
    while let Some(flag) = args.next() {
        match flag.as_str() {
            ...
            "--threads" | "-t" => {
                threads = args.next().and_then(|s| s.parse().ok()).unwrap_or(threads)
            }
            "--duration" | "-d" => {
                duration_secs = args
                    .next()
...
fn main() {
    ...
    eprintln!(
        "  target_qps : {} per thread ({} total)",
        args.target_qps,
        args.target_qps * args.threads as f64
    );
    ...
    eprintln!(
        ...
        baseline_orders,
        final_orders,
        final_orders as i64 - baseline_orders as i64
    );
    eprintln!(
        "    lineitem : {:>6} -> {:>6}  (Δ {:+})",
        baseline_lineitem,
        final_lineitem,
        final_lineitem as i64 - baseline_lineitem as i64
    );
    eprintln!(
        "    customer : {:>6} -> {:>6}  (Δ {:+})",
        baseline_customer,
        final_customer,
        final_customer as i64 - baseline_customer as i64
    );
    eprintln!(
        "    supplier : {:>6} -> {:>6}  (Δ {:+})",
        baseline_supplier,
        final_supplier,
        final_supplier as i64 - baseline_supplier as i64
    );
    eprintln!(
        "    part     : {:>6} -> {:>6}  (Δ {:+})",
        baseline_part,
        final_part,
        final_part as i64 - baseline_part as i64
    );
    eprintln!(
        "    nation   : {:>6} -> {:>6}  (Δ {:+})",
        baseline_nation,
