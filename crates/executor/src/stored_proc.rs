...
impl StoredProcExecutor {
        ...
        match statement {
            ...
            sqlrustgo_parser::Statement::AlterTable(alter_table) => {
                ...
                match &alter_table.operation {
                    ...
                    sqlrustgo_parser::AlterTableOperation::RenameColumn { name, new_name } => {
                        storage
                            .rename_column(table_name, name, new_name)
                            .map_err(|e| format!("Failed to rename column: {}", e))?;
                    }
                    sqlrustgo_parser::AlterTableOperation::AlterColumn { name, op } => match op {
                        sqlrustgo_parser::AlterColumnOperation::SetDataType { .. } => {
                            return Err(format!(
                        ...
                        sqlrustgo_parser::AlterColumnOperation::SetDefault { .. } => {
                            return Err(
                                "ALTER COLUMN ... SET DEFAULT not yet implemented".to_string()
                            );
                        }
                        sqlrustgo_parser::AlterColumnOperation::DropDefault => {
                            return Err(
                                "ALTER COLUMN ... DROP DEFAULT not yet implemented".to_string()
                            );
                        }
                        sqlrustgo_parser::AlterColumnOperation::DropNotNull => {
                            return Err(
                                "ALTER COLUMN ... DROP NOT NULL not yet implemented".to_string()
                            );
                        }
                    },
    ...
    fn expression_to_value(
        ...
        match expr {
            ...
            sqlrustgo_parser::Expression::JsonLiteral(s) => match serde_json::from_str(s) {
                Ok(v) => Value::Json(v),
                Err(_) => Value::Null,
            },
