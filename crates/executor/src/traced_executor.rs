...
use crate::executor::{VolIterator, VolcanoExecutor};
use crate::operator_profile::{OperatorProfile, ProfileTimer, QueryProfile, GLOBAL_PROFILER};
...
impl TracedExecutor {
    ...
    pub fn next(&mut self) -> SqlResult<Option<Vec<sqlrustgo_types::Value>>> {
        ...
        self.trace
            .add_metadata("last_call_ns", &elapsed.to_string());
...
pub fn execute_with_profiling(
    ...
    op_profile.record_execution(duration.as_nanos() as u64, rows.len(), batches);
...
mod tests {
    ...
    #[test]
    fn test_traced_executor_init() {
        let mock = Box::new(MockVolcanoExecutor::new());
        let mut query_trace = OperatorTrace::new("Test", "test");

        let mut traced = wrap_with_trace(mock, "TestOp", &mut query_trace);
        let result = traced.init();
        assert!(result.is_ok());
    }

    #[test]
    fn test_traced_executor_next() {
        let mock = Box::new(MockVolcanoExecutor::new());
        let mut query_trace = OperatorTrace::new("Test", "test");
        ...
        let result = traced.next();
        assert!(result.is_ok());
        assert!(result.unwrap().is_some());
    }

    #[test]
    fn test_traced_executor_next_empty() {
        let mock = Box::new(MockVolcanoExecutor::with_data(vec![]));
        let mut query_trace = OperatorTrace::new("Test", "test");
    ...
    fn test_traced_executor_close() {
        ...
        let result = traced.close();
        assert!(result.is_ok());
    }

    #[test]
    fn test_traced_executor_into_trace() {
        let mock = Box::new(MockVolcanoExecutor::new());
        let mut query_trace = OperatorTrace::new("Test", "test");

        let traced = wrap_with_trace(mock, "TestOp", &mut query_trace);
        let trace = traced.into_trace();

        assert_eq!(trace.operator_name, "TestOp");
    }

    #[test]
    fn test_traced_executor_schema() {
        let mock = Box::new(MockVolcanoExecutor::new());
        let mut query_trace = OperatorTrace::new("Test", "test");

        let traced = wrap_with_trace(mock, "TestOp", &mut query_trace);
        let schema = traced.schema();

        assert!(schema.fields.len() >= 0);
    }

    #[test]
    fn test_traced_executor_name() {
        let mock = Box::new(MockVolcanoExecutor::new());
        let mut query_trace = OperatorTrace::new("Test", "test");
    ...
    #[test]
    fn test_execute_with_trace_empty_result() {
        let mut mock = Box::new(MockVolcanoExecutor::with_data(vec![]));

        let (rows, trace) = execute_with_trace(&mut mock, "SELECT * FROM empty").unwrap();
    ...
    #[test]
    fn test_execute_with_profiling_empty_result() {
        let mut mock = Box::new(MockVolcanoExecutor::with_data(vec![]));

        let (rows, profile) = execute_with_profiling(&mut mock, "SELECT * FROM empty").unwrap();
    ...
    #[test]
    fn test_execute_with_trace_multiple_rows() {
        let mut mock = Box::new(MockVolcanoExecutor::with_data(vec![
            vec![sqlrustgo_types::Value::Integer(1)],
            vec![sqlrustgo_types::Value::Integer(2)],
            vec![sqlrustgo_types::Value::Integer(3)],
        ]));

        let (rows, trace) = execute_with_trace(&mut mock, "SELECT * FROM nums").unwrap();

        assert_eq!(rows.len(), 3);
    }

    #[test]
    fn test_execute_with_profiling_multiple_rows() {
        let mut mock = Box::new(MockVolcanoExecutor::with_data(vec![
            vec![sqlrustgo_types::Value::Integer(1)],
            vec![sqlrustgo_types::Value::Integer(2)],
        ]));

        let (rows, profile) = execute_with_profiling(&mut mock, "SELECT * FROM nums").unwrap();
    ...
    #[test]
    fn test_execute_with_trace_lightweight_empty() {
        let mut mock = Box::new(MockVolcanoExecutor::with_data(vec![]));

        let (rows, trace) =
            execute_with_trace_lightweight(&mut mock, "SELECT * FROM empty").unwrap();
