...
fn test_compile_column_not_found() {
    let filter = compile(&col("missing"));
...
fn test_compile_unary_not() {
    ...
    let filter = compile(&Expr::UnaryExpr {
        op: Operator::Not,
...
fn test_compile_qualified_wildcard() {
    let filter = compile(&Expr::QualifiedWildcard {
...
fn test_compile_fallback_aggregate_function_expr() {
    let filter = compile(&Expr::AggregateFunction {
...
fn test_eval_bool_op_eq() {
    // 1 == 1 -> true
    let filter = compile(&Expr::BinaryExpr {
...
fn test_compile_optional_some() {
    let opt = compile_optional(Some(&Expr::Literal(Value::Integer(1))));
    assert!(opt.is_some());
    let record = make_record(&[]);
