...
use sqlrustgo_executor::{mock_storage::MockStorage, test_data::{TestDataGenerator, TestDataSet, TestTableBuilder, RowBuilder}, harness::{TestHarness, helpers, assertions, fixtures, compare, ExecutorTestCase}, ExecutorResult};

use sqlrustgo_planner::{DataType, Field, PhysicalPlan, SeqScanExec, Schema};
...
fn test_mock_storage_insert() {
    ...
    storage.insert("test", vec![vec![Value::Integer(1)]]).unwrap();
