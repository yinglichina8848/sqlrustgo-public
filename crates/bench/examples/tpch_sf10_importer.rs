...
fn import_nation_region(conn: &Connection) {
    ...
    .unwrap();
    conn.execute(
        "INSERT INTO region VALUES (1, 'AMERICA', 'alanced eage')",
        [],
    )
    .unwrap();
    conn.execute("INSERT INTO region VALUES (2, 'ASIA', 'ent deposits')", [])
        .unwrap();
    conn.execute("INSERT INTO region VALUES (3, 'EUROPE', 'ically cold')", [])
        .unwrap();
    conn.execute(
        "INSERT INTO region VALUES (4, 'MIDDLE EAST', 'quires acros')",
        [],
    )
    .unwrap();
...
fn import_supplier(conn: &Connection, scale: usize) {
    ...
    let batch_size = 10000;
    let mut batch: Vec<(i32, String, String, i32, String, f64, String)> =
        Vec::with_capacity(batch_size);
...
fn insert_supplier_batch(
    conn: &Connection,
    batch: &[(i32, String, String, i32, String, f64, String)],
) {
    let mut stmt = conn
...
fn import_partsupp(conn: &Connection, scale: usize) {
    ...
    println!(
        "Importing partsupp ({} parts, 1-4 suppliers each)...",
        num_parts
    );
...
fn import_customer(conn: &Connection, scale: usize) {
    ...
    let batch_size = 10000;
    let mut batch: Vec<(i32, String, String, i32, String, f64, String, String)> =
        Vec::with_capacity(batch_size);
...
fn insert_customer_batch(
    conn: &Connection,
    batch: &[(i32, String, String, i32, String, f64, String, String)],
) {
    let mut stmt = conn
...
fn import_orders(conn: &Connection, scale: usize) {
    ...
    let batch_size = 10000;
    let mut batch: Vec<(i32, i32, String, f64, String, String, String, i32, String)> =
        Vec::with_capacity(batch_size);
...
fn insert_orders_batch(
    conn: &Connection,
    batch: &[(i32, i32, String, f64, String, String, String, i32, String)],
) {
    let mut stmt = conn
...
fn print_progress(current: usize, total: usize, elapsed_secs: f64) {
    ...
    print!(
        "\r  Progress: {:6.2}% | {:>12} / {:>12} rows | {:.0} rows/s | ETA: {:.0}s  ",
