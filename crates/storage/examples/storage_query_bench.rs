...
fn main() {
    ...
    for (table, expected) in tables {
        ...
        println!("{:>7} rows in {:>8.2}ms ({:>10.0} rows/s) {}",
                 rows.len(), elapsed.as_secs_f64() * 1000.0, rate, status);
    ...
    println!("Overall rate: {:.0} rows/s", total_rows as f64 / total_elapsed.as_secs_f64());
