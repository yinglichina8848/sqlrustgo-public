...
fn main() {
    ...
    let start = Instant::now();
    for i in 1..=100 {
        let row = vec![Value::Integer(i as i64), Value::Text(format!("user_{}", i))];
        storage
            .insert("users", vec![row])
            .expect("Failed to insert");
    }
    println!(
        "  Inserted in {:.2}s ({:.0} rows/s)",
    ...
    if rows.len() >= 100 {
        println!(
            "✅ FileStorage persistence VERIFIED: {} rows persisted",
            rows.len()
        );
    } else {
