...
fn main() {
    ...
    println!(
        "  Throughput: {:.0} rows/s",
        rows.len() as f64 / json_time.as_secs_f64()
    );
    drop(storage);
    ...
    let raw_path = PathBuf::from(
        "/Users/liying/workspace/dev/heartopen/SQLRustGo/data/tpch-sf01-generated/lineitem.tbl",
    );
    ...
    println!(
        "  Scanned {} lines in {:.2}ms",
        line_count,
        raw_time.as_secs_f64() * 1000.0
    );
    println!(
        "  Throughput: {:.0} lines/s",
        line_count as f64 / raw_time.as_secs_f64()
    );
