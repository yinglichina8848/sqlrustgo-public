...
use sqlrustgo_storage::{FileStorage, TableInfo, ColumnDefinition, TableData};
...
fn main() {
    ...
    for line in reader.lines() {
        ...
        if fields.len() >= 2 {
            ...
            if batch.len() >= 10000 {
                    ...
                    info: TableInfo {
                        ...
                        columns: vec![
                            ColumnDefinition {
                                ...
                                nullable: false, is_unique: false, is_primary_key: false,
                                references: None, auto_increment: false,
                            },
                            ColumnDefinition {
                ...
                storage.insert_table("lineitem".to_string(), table_data).unwrap();
                count += 1;
                
                if count % 100 == 0 {
                    let elapsed = start.elapsed().as_secs_f64();
                    println!("  {} rows imported ({:.0} rows/s)", total_rows, total_rows as f64 / elapsed);
                }
            }
        }
    }
    ...
    println!("\n==============================================");
    println!("Import complete: {} rows in {:.1}s", total_rows, import_time.as_secs_f64());
    println!("Throughput: {:.0} rows/s", total_rows as f64 / import_time.as_secs_f64());
    println!("==============================================");
}
