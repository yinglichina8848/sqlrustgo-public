...
fn main() {
    ...
    println!("  JSON load: {} rows in {:.2}ms", rows.len(), json_time.as_secs_f64() * 1000.0);
    
    // Save as binary
    println!("\n[2] Saving as binary...");
    let binary_dir = PathBuf::from("/tmp/binary_storage");
    let binary_storage = BinaryTableStorage::new(binary_dir.clone()).expect("Failed to create binary storage");
    ...
    let start = Instant::now();
    binary_storage.save("lineitem", &table_data).expect("Failed to save binary");
    let binary_save_time = start.elapsed();
    println!("  Binary save: {} rows in {:.2}ms", rows_count, binary_save_time.as_secs_f64() * 1000.0);
    
    // Load from binary
    println!("\n[3] Loading from binary...");
    let binary_storage = BinaryTableStorage::new(binary_dir).expect("Failed to open binary storage");
    let start = Instant::now();
    let loaded = binary_storage.load("lineitem").expect("Failed to load binary");
    let binary_load_time = start.elapsed();
    println!("  Binary load: {} rows in {:.2}ms", loaded.rows.len(), binary_load_time.as_secs_f64() * 1000.0);
