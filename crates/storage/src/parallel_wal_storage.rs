...
use crate::engine::{Record, SqlResult, SqlError, StorageEngine, TableInfo, ColumnDefinition};
...
impl<S: StorageEngine, W: WalManager> StorageEngine for ParallelWalStorage<S, W> {
    ...
    fn update(&mut self, table: &str, filters: &[crate::engine::Value], updates: &[(usize, crate::engine::Value)]) -> SqlResult<usize> {
        self.inner.update(table, filters, updates)
    }
    
    fn create_table(&mut self, info: &TableInfo) -> SqlResult<()> { self.inner.create_table(info) }
    fn drop_table(&mut self, table: &str) -> SqlResult<()> { self.inner.drop_table(table) }
    fn has_table(&self, table: &str) -> bool { self.inner.has_table(table) }
    fn list_tables(&self) -> Vec<String> { self.inner.list_tables() }
    fn get_table_info(&self, table: &str) -> SqlResult<TableInfo> { self.inner.get_table_info(table) }
    
    fn flush(&mut self) -> SqlResult<()> { self.inner.flush() }
    ...
    fn in_transaction(&self) -> bool { self.inner.in_transaction() }
    fn set_current_tx_id(&mut self, tx_id: u64) { self.current_tx_id = tx_id; }
    fn is_wal_enabled(&self) -> bool { self.wal_enabled }
    
    fn delete_if(&mut self, table: &str, filter: &crate::engine::RowFilter) -> SqlResult<usize> {
        self.inner.delete_if(table, filter)
    }
    
    fn update_if(&mut self, table: &str, filter: &crate::engine::RowFilter, mutation: &crate::engine::RowMutation) -> SqlResult<usize> {
    ...
    fn drop_index(&mut self, table: &str, column: &str) -> SqlResult<()> { self.inner.drop_index(table, column) }
    fn add_column(&mut self, table: &str, column: ColumnDefinition) -> SqlResult<()> { self.inner.add_column(table, column) }
    fn rename_table(&mut self, table: &str, new_name: &str) -> SqlResult<()> { self.inner.rename_table(table, new_name) }
    fn create_trigger(&mut self, trigger: crate::engine::TriggerInfo) -> SqlResult<()> { self.inner.create_trigger(trigger) }
    fn drop_trigger(&mut self, name: &str) -> SqlResult<()> { self.inner.drop_trigger(name) }
    fn get_trigger(&self, name: &str) -> Option<crate::engine::TriggerInfo> { self.inner.get_trigger(name) }
    fn list_triggers(&self, table: &str) -> Vec<crate::engine::TriggerInfo> { self.inner.list_triggers(table) }
    fn list_indexes(&self, table: &str) -> Vec<(String, String)> { self.inner.list_indexes(table) }
    fn has_view(&self, name: &str) -> bool { self.inner.has_view(name) }
}
