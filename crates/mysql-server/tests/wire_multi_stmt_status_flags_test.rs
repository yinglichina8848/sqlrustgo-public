...
fn com_query_multi_stmt_single_statement_does_not_set_more_results_flag() {
    ...
    assert_eq!(
        results.len(),
        1,
        "single SELECT must return exactly one ResultSet"
    );
    match &results[0] {
        sqlrustgo_mysql_client::ResultSet::Select { status_flags, .. } => {
            assert_eq!(
...
fn com_query_multi_stmt_dml_returns_ok_packets_with_more_results_flag() {
    ...
    match &results[0] {
        ...
        other => panic!(
            "expected ResultSet::Ok for CREATE, got {:?}",
            std::mem::discriminant(other)
        ),
    }
    match &results[1] {
        ...
        other => panic!(
            "expected ResultSet::Ok for INSERT, got {:?}",
            std::mem::discriminant(other)
        ),
    }
    match &results[2] {
        ...
        other => panic!(
            "expected ResultSet::Select for SELECT, got {:?}",
            std::mem::discriminant(other)
        ),
    }
}
