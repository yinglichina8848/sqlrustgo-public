use sqlrustgo_transaction::{DistributedLockManager, SsiDetector, SsiDetectorSync, TxId};
use std::sync::Arc;

#[tokio::test]
async fn test_ssi_concurrent_read_no_conflict() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"key1".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"key1".to_vec()).await;

    assert!(detector.validate_commit(tx1).await.is_ok());
    assert!(detector.validate_commit(tx2).await.is_ok());
}

#[tokio::test]
async fn test_ssi_rw_conflict_detection() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    // Dangerous structure: T1: R(X), W(Y) and T2: R(Y), W(X)
    // Both will have RW-WR conflicts when validating
    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec()).await;
    let _ = detector.record_write(tx1, b"Y".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"Y".to_vec()).await;
    let _ = detector.record_write(tx2, b"X".to_vec()).await;

    // One of them should fail (whichever validates second finds the cycle)
    let result1 = detector.validate_commit(tx1).await;
    let result2 = detector.validate_commit(tx2).await;

    // At least one should fail due to cycle detection
    assert!(result1.is_err() || result2.is_err());
}

#[tokio::test]
async fn test_ssi_write_no_conflict() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    let _ = detector.record_write(tx1, b"key1".to_vec()).await;

    let tx2 = TxId::new(2);
    let _ = detector.record_write(tx2, b"key2".to_vec()).await;

    assert!(detector.validate_commit(tx1).await.is_ok());
    assert!(detector.validate_commit(tx2).await.is_ok());
}

#[test]
fn test_ssi_sync_concurrent_read_no_conflict() {
    let detector = SsiDetectorSync::new();

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"key1".to_vec());

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"key1".to_vec());

    assert!(detector.validate_commit(tx1).is_ok());
    assert!(detector.validate_commit(tx2).is_ok());
}

#[test]
fn test_ssi_sync_rw_conflict_detection() {
    let detector = SsiDetectorSync::new();

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec());
    detector.record_write(tx1, b"Y".to_vec());

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"Y".to_vec());
    detector.record_write(tx2, b"X".to_vec());

    let result1 = detector.validate_commit(tx1);
    let result2 = detector.validate_commit(tx2);

    assert!(result1.is_err() || result2.is_err());
}

#[test]
fn test_ssi_sync_write_no_conflict() {
    let detector = SsiDetectorSync::new();

    let tx1 = TxId::new(1);
    detector.record_write(tx1, b"key1".to_vec());

    let tx2 = TxId::new(2);
    detector.record_write(tx2, b"key2".to_vec());

    assert!(detector.validate_commit(tx1).is_ok());
    assert!(detector.validate_commit(tx2).is_ok());
}

#[tokio::test]
async fn test_ssi_write_skew_detection() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec()).await;
    let _ = detector.record_write(tx1, b"Y".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"Y".to_vec()).await;
    let _ = detector.record_write(tx2, b"X".to_vec()).await;

    let result1 = detector.validate_commit(tx1).await;
    let result2 = detector.validate_commit(tx2).await;

    assert!(result1.is_err() || result2.is_err());
}

#[tokio::test]
async fn test_ssi_direct_write_conflict() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    let _ = detector.record_write(tx1, b"X".to_vec()).await;

    let tx2 = TxId::new(2);
    let _ = detector.record_write(tx2, b"X".to_vec()).await;

    let result1 = detector.validate_commit(tx1).await;
    let result2 = detector.validate_commit(tx2).await;

    assert!(result1.is_ok() || result2.is_ok());
}

#[tokio::test]
async fn test_ssi_multiple_readers_no_conflict() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"key1".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"key1".to_vec()).await;

    let tx3 = TxId::new(3);
    detector.record_read(tx3, b"key1".to_vec()).await;

    assert!(detector.validate_commit(tx1).await.is_ok());
    assert!(detector.validate_commit(tx2).await.is_ok());
    assert!(detector.validate_commit(tx3).await.is_ok());
}

#[test]
fn test_ssi_sync_write_skew_detection() {
    let detector = SsiDetectorSync::new();

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec());
    let _ = detector.record_write(tx1, b"Y".to_vec());

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"Y".to_vec());
    let _ = detector.record_write(tx2, b"X".to_vec());

    let result1 = detector.validate_commit(tx1);
    let result2 = detector.validate_commit(tx2);

    assert!(result1.is_err() || result2.is_err());
}

#[tokio::test]
async fn test_ssi_validation_order_matters() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec()).await;
    detector.record_read(tx1, b"Y".to_vec()).await;
    let _ = detector.record_write(tx1, b"Z".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"X".to_vec()).await;
    detector.record_read(tx2, b"Y".to_vec()).await;
    let _ = detector.record_write(tx2, b"W".to_vec()).await;

    let result1 = detector.validate_commit(tx1).await;
    let result2 = detector.validate_commit(tx2).await;

    assert!(result1.is_ok() && result2.is_ok());
}

#[tokio::test]
async fn test_ssi_abort_prevents_conflict() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec()).await;
    let _ = detector.record_write(tx1, b"Y".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"Y".to_vec()).await;
    let _ = detector.record_write(tx2, b"X".to_vec()).await;

    detector.release(tx1).await;

    assert!(detector.validate_commit(tx2).await.is_ok());
}

#[tokio::test]
async fn test_ssi_read_after_write_in_same_tx() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    let _ = detector.record_write(tx1, b"X".to_vec()).await;
    detector.record_read(tx1, b"X".to_vec()).await;

    assert!(detector.validate_commit(tx1).await.is_ok());
}

#[tokio::test]
async fn test_ssi_overlapping_reads_different_writes() {
    let locks = Arc::new(DistributedLockManager::new());
    let detector = SsiDetector::new(locks);

    let tx1 = TxId::new(1);
    detector.record_read(tx1, b"X".to_vec()).await;
    detector.record_read(tx1, b"Y".to_vec()).await;
    let _ = detector.record_write(tx1, b"Z".to_vec()).await;

    let tx2 = TxId::new(2);
    detector.record_read(tx2, b"X".to_vec()).await;
    detector.record_read(tx2, b"Y".to_vec()).await;
    let _ = detector.record_write(tx2, b"W".to_vec()).await;

    assert!(detector.validate_commit(tx1).await.is_ok());
    assert!(detector.validate_commit(tx2).await.is_ok());
}
