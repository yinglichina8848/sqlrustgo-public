...
pub fn generate_tpch_data(scale_factor: f64) -> Arc<RwLock<MemoryStorage>> {
...
// ============================================================================
// Real TPC-H .tbl file loader
// TPC-H .tbl format: pipe-delimited, trailing pipe, no header
// ============================================================================

pub struct TblDataLoader {
    data_dir: String,
}
...
impl TblDataLoader {
    ...
    /// Load real TPC-H SF=1 data from .tbl files
    pub fn load_sf1(&self) -> std::io::Result<Arc<RwLock<MemoryStorage>>> {
        let mut storage = MemoryStorage::new();

        self.load_table(&mut storage, "nation")?;
        self.load_table(&mut storage, "region")?;
        self.load_customer(&mut storage)?;
        self.load_supplier(&mut storage)?;
        self.load_part(&mut storage)?;
        self.load_partsupp(&mut storage)?;
        self.load_orders(&mut storage)?;
        self.load_lineitem(&mut storage)?;

        println!("Real SF=1 data loaded successfully");
        Ok(Arc::new(RwLock::new(storage)))
    }

    /// Fallback: generate mock 1% scale data
    pub fn generate_sf1(&self) -> Arc<RwLock<MemoryStorage>> {
        generate_tpch_data(0.01)
    }

    fn load_table(&self, storage: &mut MemoryStorage, table: &str) -> std::io::Result<()> {
        let path = format!("{}/{}_clean.tbl", self.data_dir, table);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            let fields: Vec<&str> = trimmed.split('|').collect();
            let values: Vec<sqlrustgo_types::Value> = fields.iter().map(|s| self.parse_value(s)).collect();
            rows.push(values);
        }
        let row_count = rows.len();
        storage.insert(table, rows).ok();
        println!("  {}: {} rows", table, row_count);
        Ok(())
    }

    fn load_customer(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/customer_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            let fields: Vec<&str> = trimmed.split('|').collect();
            rows.push(vec![
                self.parse_int(fields[0]),
                self.parse_text(fields[1]),
                self.parse_text(fields[2]),
                self.parse_int(fields[3]),
                self.parse_text(fields[4]),
                self.parse_float(fields[5]),
                self.parse_text(fields[6]),
                self.parse_text(fields[7]),
            ]);
        }
        let row_count = rows.len();
        storage.insert("customer", rows).ok();
        println!("  customer: {} rows", row_count);
        Ok(())
    }

    fn load_supplier(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/supplier_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
        ...
        let row_count = rows.len();
        storage.insert("supplier", rows).ok();
        println!("  supplier: {} rows", row_count);
        Ok(())
    }

    fn load_part(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/part_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            ...
            rows.push(vec![
                ...
                self.parse_text(fields[1]),
                self.parse_text(fields[2]),
                self.parse_text(fields[3]),
                self.parse_int(fields[4]),
                self.parse_text(fields[5]),
                self.parse_int(fields[6]),
                self.parse_text(fields[7]),
                self.parse_float(fields[8]),
            ]);
        }
        let row_count = rows.len();
        storage.insert("part", rows).ok();
        println!("  part: {} rows", row_count);
        Ok(())
    }

    fn load_partsupp(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/partsupp_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            ...
            rows.push(vec![
                self.parse_int(fields[0]),
                self.parse_int(fields[1]),
                self.parse_int(fields[2]),
                self.parse_float(fields[3]),
                self.parse_text(fields[4]),
            ]);
        }
        let row_count = rows.len();
        storage.insert("partsupp", rows).ok();
        println!("  partsupp: {} rows", row_count);
        Ok(())
    }

    fn load_orders(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/orders_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            ...
            rows.push(vec![
                ...
                self.parse_text(fields[5]),
                self.parse_text(fields[6]),
                self.parse_int(fields[7]),
                self.parse_text(fields[8]),
            ]);
        }
        let row_count = rows.len();
        storage.insert("orders", rows).ok();
        println!("  orders: {} rows", row_count);
        Ok(())
    }

    fn load_lineitem(&self, storage: &mut MemoryStorage) -> std::io::Result<()> {
        let path = format!("{}/lineitem_clean.tbl", self.data_dir);
        let content = std::fs::read_to_string(&path)?;
        let mut rows = Vec::new();
        for line in content.lines() {
            let trimmed = line.trim_end_matches('|');
            ...
            rows.push(vec![
                ...
                self.parse_int(fields[2]),
                self.parse_int(fields[3]),
                self.parse_float(fields[4]),
                self.parse_float(fields[5]),
                self.parse_float(fields[6]),
                self.parse_float(fields[7]),
                self.parse_text(fields[8]),
                self.parse_text(fields[9]),
                self.parse_text(fields[10]),
                self.parse_text(fields[11]),
                self.parse_text(fields[12]),
            ]);
        }
        let row_count = rows.len();
        storage.insert("lineitem", rows).ok();
        println!("  lineitem: {} rows", row_count);
        Ok(())
    }

    fn parse_int(&self, s: &str) -> sqlrustgo_types::Value {
        sqlrustgo_types::Value::Integer(s.parse().unwrap_or(0))
    }

    fn parse_float(&self, s: &str) -> sqlrustgo_types::Value {
        sqlrustgo_types::Value::Float(s.parse().unwrap_or(0.0))
    }

    fn parse_text(&self, s: &str) -> sqlrustgo_types::Value {
        sqlrustgo_types::Value::Text(s.to_string())
    }

    fn parse_value(&self, s: &str) -> sqlrustgo_types::Value {
