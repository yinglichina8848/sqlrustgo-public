...
| 账号 | 角色 | 权限层 | 职责 |
|------|------|--------|------|
| **heartopen** | 功能开发 | L1 | 实现功能、创建 PR |
| **openheart** | 架构开发 | L1 | 优化架构、技术债处理 |
| **maintainer** | 稳定维护/审核 | L2 | 审核 PR、代码质量把关 |
| **yinglichina8848** | 计划制定/调度/发布 | L3 | 版本计划、发布控制 |
...
| 规则 | 说明 |
|------|------|
| 开发身份创建 PR | heartopen/openheart |
| 不允许作者自审 | 必须由其他身份审核 |
| 审核身份批准 | maintainer |
| 发布身份控制 merge | yinglichina8848 |
...
| Agent | 职责 | 权限层 | 可访问目录 |
|-------|------|--------|------------|
| **heartopen** | 实现功能、创建 PR | L1 | dev/heartopen |
| **openheart** | 优化架构、技术债处理 | L1 | dev/openheart |
| **maintainer** | 审核 PR、代码质量把关 | L2 | maintainer |
| **yinglichina8848** | 制定版本计划、拆分任务、控制 release gate | L3 | yinglichina |
...
| 风险 | 影响 | 概率 | 缓解措施 | 责任人 |
|------|------|------|----------|--------|
| 身份混用 | 🔴 高 | 🟡 中 | 目录隔离 + local config | yinglichina8848 |
| 自审行为 | 🔴 高 | 🟢 低 | 自动检测 + 流程规范 | maintainer |
| Token 泄露 | 🔴 高 | 🟢 低 | 独立目录 + 权限限制 | yinglichina8848 |
| 绕过 PR | 🔴 高 | 🟢 低 | 分支保护 + CI 强制 | yinglichina8848 |
| 发布失误 | 🟡 中 | 🟢 低 | 手动确认 + 审核流程 | yinglichina8848 |
...
| 场景 | 响应步骤 | 责任人 |
|------|----------|--------|
| Token 泄露 | 1. 立即撤销 token<br>2. 生成新 token<br>3. 审计相关操作 | yinglichina8848 |
| 自审发现 | 1. 撤销审核<br>2. 重新分配审核人<br>3. 记录事件 | maintainer |
| 发布失误 | 1. 回滚 release<br>2. 修复问题<br>3. 重新发布 | yinglichina8848 |
