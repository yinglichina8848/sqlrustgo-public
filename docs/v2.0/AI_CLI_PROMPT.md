...
> 版本：v1.3
> 日期：2026-03-02
> **当前执行口径（2026-03-07）**: v1.2.0 当前开发分支为 `develop/v1.2.0`
...
⚠️ **重要**: v1.2.0 开发必须在 `develop/v1.2.0` 分支进行，不要在 `release/v1.0.0` 或 `release/v1.1.0` 上开发！
...
### 测试覆盖率提升 (新)
- #101: 测试覆盖率提升至 90%
  - COV-001: planner/mod.rs 测试
  - COV-002: planner/logical_plan.rs 测试
  - COV-003: planner/physical_plan.rs 测试
  - COV-004: planner/executor.rs 测试
  - COV-005: parser/mod.rs 测试补充
  - COV-006: types/value.rs 测试补充
  - COV-007 ~ COV-009: CI 集成
...
| Issue | 任务 | 文件 | 预估时间 |
|-------|------|------|----------|
| #101 | COV-001: planner/mod.rs 测试 | src/planner/mod.rs | 2h |
| #101 | COV-002: logical_plan.rs 测试 | src/planner/logical_plan.rs | 3h |
| #101 | COV-003: physical_plan.rs 测试 | src/planner/physical_plan.rs | 4h |
...
Step 5: 开始编码 → 实现 + 测试
步骤6：提交 PR → develop/v1.2.0 --head <branch>
```
...
2. **及时同步**: 开始前 `git pull origin develop/v1.2.0`
3. **质量保证**: `cargo test` + `cargo clippy` 通过再提交
4. **分支正确**: PR 必须指向 `develop/v1.2.0`
