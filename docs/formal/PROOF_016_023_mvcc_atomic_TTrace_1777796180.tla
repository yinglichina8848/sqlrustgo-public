---- MODULE PROOF_016_023_mvcc_atomic_TTrace_1777796180 ----
...
_init ==
    /\ committedWrites = _TETrace[1].committedWrites
...
_next ==
    /\ \E i,j \in DOMAIN _TETrace:
        /\ \/ /\ j = i + 1
              /\ i = TLCGet("level")
        /\ committedWrites  = _TETrace[i].committedWrites
        /\ committedWrites' = _TETrace[j].committedWrites
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("PROOF_016_023_mvcc_atomic_TTrace_1777796180.json", _TETrace)
...
expression == 
    [
        ...
        \* ,_committedWritesUnchanged |-> committedWrites = committedWrites'
        
        \* Format the `committedWrites` variable as Json value.
        \* ,_committedWritesJson |->
        \*     LET J == INSTANCE Json
        \*     IN J!ToJson(committedWrites)
        
        \* Lastly, you may build expressions over arbitrary sets of states by
        \* leveraging the _TETrace operator.  For example, this is how to
        \* count the number of times a spec variable changed up to the current
        \* state in the trace.
        \* ,_committedWritesModCount |->
        \*     LET F[s \in DOMAIN _TETrace] ==
        \*         IF s = 1 THEN 0
        \*         ELSE IF _TETrace[s].committedWrites # _TETrace[s-1].committedWrites
...
\*
\*trace == IODeserialize("PROOF_016_023_mvcc_atomic_TTrace_1777796180.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    <<
    ([committed |-> {},writeSet |-> [T1 |-> {}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],committedWrites |-> {},waitFor |-> [T1 |-> {}, T2 |-> {}]]),
    ([committed |-> {"T1"},writeSet |-> [T1 |-> {}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],committedWrites |-> {},waitFor |-> [T1 |-> {}, T2 |-> {}]]),
    ([committed |-> {"T1"},writeSet |-> [T1 |-> {"K1"}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],committedWrites |-> {},waitFor |-> [T1 |-> {}, T2 |-> {}]]),
    ([committed |-> {"T1", "T2"},writeSet |-> [T1 |-> {"K1"}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],committedWrites |-> {},waitFor |-> [T1 |-> {}, T2 |-> {}]]),
    ([committed |-> {"T1", "T2"},writeSet |-> [T1 |-> {"K1"}, T2 |-> {"K1"}],readSet |-> [T1 |-> {}, T2 |-> {}],committedWrites |-> {},waitFor |-> [T1 |-> {}, T2 |-> {}]])
    >>
----
...
---- CONFIG PROOF_016_023_mvcc_atomic_TTrace_1777796180 ----
CONSTANTS
    Txns = { "T1" , "T2" }
    Keys = { "K1" }
...
ALIAS
    _expression
=============================================================================
\* Generated on Sun May 03 16:16:21 CST 2026