---- MODULE PROOF_016_023_mvcc_atomic_TTrace_1777795984 ----
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("PROOF_016_023_mvcc_atomic_TTrace_1777795984.json", _TETrace)
...
\*
\*trace == IODeserialize("PROOF_016_023_mvcc_atomic_TTrace_1777795984.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([committed |-> {},writeSet |-> [T1 |-> {}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],waitFor |-> [T1 |-> {"T2"}, T2 |-> {}],ts |-> [T1 |-> "T1", T2 |-> "T2"]]),
    ([committed |-> {},writeSet |-> [T1 |-> {"K1"}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],waitFor |-> [T1 |-> {"T2"}, T2 |-> {}],ts |-> [T1 |-> "T1", T2 |-> "T2"]]),
    ([committed |-> {},writeSet |-> [T1 |-> {"K1"}, T2 |-> {}],readSet |-> [T1 |-> {}, T2 |-> {}],waitFor |-> [T1 |-> {"T2"}, T2 |-> {"T1"}],ts |-> [T1 |-> "T1", T2 |-> "T2"]])
    >>
----
...
---- CONFIG PROOF_016_023_mvcc_atomic_TTrace_1777795984 ----
CONSTANTS
    Txns = { "T1" , "T2" }
    Keys = { "K1" }
...
ALIAS
    _expression
=============================================================================
\* Generated on Sun May 03 16:13:05 CST 2026