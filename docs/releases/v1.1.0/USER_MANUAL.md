...
| **服务器模式** | `cargo run -- --server` | Client-Server 架构 |
...
| 错误 | 原因 | 解决方法 |
|------|------|----------|
| `Table not found` | 表不存在 | 先创建表 |
| `Column not found` | 列名错误 | 检查列名 |
| `Duplicate key` | 主键冲突 | 使用不同的主键值 |
| `Syntax error` | SQL 语法错误 | 检查 SQL 语法 |
