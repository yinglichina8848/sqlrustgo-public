...
**source_run**: v312-11-verification-2026-08-10
**timestamp**: 2026-08-10T00:40:00+08:00
...
|-----------|--------|----------|
| Gate script exists | ✅ PASS | scripts/gate/check_sqllogictest_v312.sh |
| Cargo build passes | ✅ PASS | cargo build -p sqlrustgo_sqllogictest |
| Runner smoke passes | ✅ PASS | 4 PASS, 0 FAIL |
| exclusions.yml exists | ✅ PASS | docs/releases/v3.12.0/sqllogictest-baseline/exclusions.yml |
| exclusions.yml not seed | ✅ PASS | 16 items, active status |
...
The V312-11 gate establishes a smoke baseline for the SQLLogicTest oracle. All required files are in place, exclusions are properly tracked with OpenSpec changes, and the gate script executes successfully.
...
```
$(cat docs/releases/v3.12.0/sqllogictest-baseline/exclusions.yml docs/releases/v3.12.0/sqllogictest-baseline/sqlite-corpus-manifest.json | md5sum)
```
