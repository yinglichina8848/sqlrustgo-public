# Issue #4676 — Per-Issue Evidence (V312-RC-GA Triage)
...
> source_repo=openclaw/sqlrustgo, source_run=v312-rc-ga-phase1.3-4676,
> policy=Anti-Fabrication-Policy-v1.0
>
> **supersedes:** none (initial creation)
> **superseded by:** none yet
> **linked branch plan:** PR-B5 (per Path B execution plan §2)

| Field | Value |
|-------|-------|
| Issue | [#4676](http://192.168.0.252:3000/openclaw/sqlrustgo/issues/4676) |
| Title | [executor] 数学函数 MOD/POWER/LOG/EXP/SQRT 完全未实现 (返回空) |
...
1. PR (PR-B5) merged into `develop/v3.12.0` with at least one regression test that fails RED before merge.
...
3. Verifier command output (real exit code + stdout snippet + SHA-256) saved to
   `docs/releases/v3.12.0/logs/pr-4676_{short}_{commit}_{timestamp}.log`.
4. `CLAIM_DOWNGRADE_MANIFEST.md` n-class entry #4676 entry updated to reflect the now-removed limitation.
5. Gitea issue state transitioned via `PATCH /issues/4676` to `state=closed`
   with a comment citing the merge commit SHA.
6. Label replaced: `GA-blocker` (or `GA-claim-caveat`) → `closed-by-pr-4676`.
...
- B-track teaching acceptance will fail at week-4676-caveat content.
...
```bash
# Pre-condition: PR-PR-B5 merged; merge commit known.
MERGE_COMMIT="$(git log --oneline --merges develop/v3.12.0 | grep -iE 'issue-?4676' | head -1 | awk '{print $1}')"
test -n "$MERGE_COMMIT" || { echo "FATAL: no merge commit found for issue #4676" >&2; exit 1; }

# Verify regression test exists in tree
find tests -name "*[executor]_数学函数_MOD_POWER_LOG_EXP_SQRT_完全未实现_返回空*" -type f | head -1 | grep -q . || { echo "FAIL: no regression test for #4676"; exit 1; }

# Run regression test in isolation
cargo test --all-features -- '*issue-?4676*' 2>&1 | tail -3   | tee /tmp/issue-4676-test.log   | grep -qE "test result: ok|passed"   || { echo "FAIL: regression test for #4676 did not pass"; exit 1; }

# SHA-256 of test log
sha256sum /tmp/issue-4676-test.log | tee /tmp/issue-4676-test.sha256
...
- Claim downgrade entry: `docs/releases/v3.12.0/CLAIM_DOWNGRADE_MANIFEST.md` §3 entry #4676
- Path B plan: `docs/plans/2026-09-04-v312-rc-ga-path-b-execution-plan.md` §2 row PR-B5
...
| PR PR-B5 merge commit (after merge) | TBD |
| Regression test log (after fix) | TBD |
| Oracle diff log (after fix) | TBD |

*When PR-B5 lands and the four artifacts are filled, this hash section is updated,
