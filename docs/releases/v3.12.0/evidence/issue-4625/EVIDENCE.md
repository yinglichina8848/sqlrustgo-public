# Issue #4625 — Per-Issue Evidence (V312-RC-GA Triage)
...
> source_repo=openclaw/sqlrustgo, source_run=v312-rc-ga-phase1.3-4625,
> policy=Anti-Fabrication-Policy-v1.0
>
> **supersedes:** none (initial creation)
> **superseded by:** none yet
> **linked branch plan:** PR-B9 (per Path B execution plan §2)

| Field | Value |
|-------|-------|
| Issue | [#4625](http://192.168.0.252:3000/openclaw/sqlrustgo/issues/4625) |
| Title | [optimizer] INDEXED BY hint 不被优化器尊重, 全表扫描 |
...
1. PR (PR-B9) merged into `develop/v3.12.0` with at least one regression test that fails RED before merge.
...
3. Verifier command output (real exit code + stdout snippet + SHA-256) saved to
   `docs/releases/v3.12.0/logs/pr-4625_{short}_{commit}_{timestamp}.log`.
4. `CLAIM_DOWNGRADE_MANIFEST.md` n-class entry #4625 entry updated to reflect the now-removed limitation.
5. Gitea issue state transitioned via `PATCH /issues/4625` to `state=closed`
   with a comment citing the merge commit SHA.
6. Label replaced: `GA-blocker` (or `GA-claim-caveat`) → `closed-by-pr-4625`.
...
- B-track teaching acceptance will fail at week-4625-caveat content.
...
```bash
# Pre-condition: PR-PR-B9 merged; merge commit known.
MERGE_COMMIT="$(git log --oneline --merges develop/v3.12.0 | grep -iE 'issue-?4625' | head -1 | awk '{print $1}')"
test -n "$MERGE_COMMIT" || { echo "FATAL: no merge commit found for issue #4625" >&2; exit 1; }

# Verify regression test exists in tree
find tests -name "*[optimizer]_INDEXED_BY_hint_不被优化器尊重,_全表扫描*" -type f | head -1 | grep -q . || { echo "FAIL: no regression test for #4625"; exit 1; }

# Run regression test in isolation
cargo test --all-features -- '*issue-?4625*' 2>&1 | tail -3   | tee /tmp/issue-4625-test.log   | grep -qE "test result: ok|passed"   || { echo "FAIL: regression test for #4625 did not pass"; exit 1; }

# SHA-256 of test log
sha256sum /tmp/issue-4625-test.log | tee /tmp/issue-4625-test.sha256
...
- Claim downgrade entry: `docs/releases/v3.12.0/CLAIM_DOWNGRADE_MANIFEST.md` §3 entry #4625
- Path B plan: `docs/plans/2026-09-04-v312-rc-ga-path-b-execution-plan.md` §2 row PR-B9
...
| PR PR-B9 merge commit (after merge) | TBD |
| Regression test log (after fix) | TBD |
| Oracle diff log (after fix) | TBD |

*When PR-B9 lands and the four artifacts are filled, this hash section is updated,
