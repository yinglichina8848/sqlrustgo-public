...
### 4.1 执行性 gate（B6_V312_47_PARTIAL_CLOSURE，2026-08-17 落地于 #4220 closure 推进）

`scripts/gate/check_beta_v3.12.0.sh` 中新增 `B6_V312_47_PARTIAL_CLOSURE`，不再依赖人工 review。机器可读验证：
1. **README PARTIAL 行扫描** — 状态表（~50-55 行）+ 功能矩阵（~121-160 行），每一行包含 `PARTIAL` 必须同时引用 `#NNNN` Gitea issue。
2. **本计划文件结构** — `## 2. README PARTIAL 整改总账` / `## 3. 新增 Issue` / `## 4. 必须同步到门禁` 三节齐备。
3. **ISSUES_PLAN.md 同步** — 引用 `#4220` (per §4.3)。

如任一不满足 → hard Beta blocker。
