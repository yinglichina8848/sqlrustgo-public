...
- **Net active count**: 31 (Section A) + 45 (Section B) = **76 active entries** (plus 2 REACTIVATED-as-audit rows = 78 total entries in this registry, 78 entries in `DISABLED_BINARIES` in `scripts/gate/run_b2_per_binary.py`, and 78 entries in `DISABLED_TESTS_LIST` in `scripts/gate/check_beta_v3.12.0.sh`).
