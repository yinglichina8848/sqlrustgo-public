...
graph TD
    ...
    class Planner,Executor,TxMgr,MVCC,WAL,Storage,UniqueIdx,CompIdx,Stats,Cache,DateType done;
    class LockMgr,Deadlock,ConnPool,Benchmark todo;
...
|------|------|------|--------|----------|--------------|
| T-04 | 并发控制 | 行级锁 (Row-Level Locking) | P0 | 300 行 | #633 ✅ |
...
| D-02 | 数据类型 | TIMESTAMP 类型 | P1 | 150 行 | #634 ✅ |
...
| 任务 | 交付物 |
|------|--------|
| LockManager 实现 | `crates/transaction/src/lock.rs` |
| Executor 接入锁 | 修改 `crates/executor/src/executor.rs` |
| 单元测试 | `crates/transaction/tests/lock_test.rs` |
