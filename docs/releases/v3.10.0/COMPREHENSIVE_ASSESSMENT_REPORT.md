...
|------|--------|---------|------|
| TPC-H SF=0.1 总时间 | ~2.3s | TBD (pending SF1 perf) | 待测 |
| 启动时间 | < 1s | < 1s | — |
| 内存占用 (idle) | ~15-30 MB | ~15-30 MB | — |
| 并行执行附加开销 | N/A | ~5-10% (短查询) | 已知 trade-off |

性能基线等待 TPC-H SF=1 完整测试数据（硬件受限）。详情见 `docs/releases/v3.10.0/perf/`。

### 7.2 已知性能风险

- 并行执行器引入线程调度开销（短查询可能退化 5-10%）
- CBO 新增路径尚未有 TPC-H SF=1 完整验证
- Gap Locking P99 延迟基线尚未收集
...
**综合评级**: B+（GA 条件全部满足，CA 签署完成，168h SOAK 实测通过）
