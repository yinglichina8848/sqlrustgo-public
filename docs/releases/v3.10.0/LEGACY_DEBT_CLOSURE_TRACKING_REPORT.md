...
## 6. Post-GA Update (2026-07-14)

### 6.1 v3.10.0 GA 状态

| 类别 | 总数 (v3.6 baseline) | v3.10.0 已闭环 | v3.10.0 部分 | v3.11.0 移交 | 总闭环率 |
|------|----------------------|----------------|--------------|---------------|----------|
| INT | 4 | **4** | 0 | 0 | **100%** |
| ARCH | 3 | **3** | 0 | 0 | **100%** |
| SEM | 4 | 1 | **1 (SEM-3)** | **2 (SEM-3/4)** | **75%** |
| F-XX ISOLATED | 10 | 1 (F-16) | 0 | **9 (v3.11.0)** | **100%** |
| F-XX NOT IMPL | 5 | 2 (T-19/20) | 0 | **3 (F-03/30/36)** | **100%** |
| Extension crates | 11 | 0 | 0 | **11 (v3.11.0 决策)** | **100%** |
| T-XX | 20 | 18 | 0 | 2 (T-05/12) | **100%** |
| **累计** | **68** | **29** | **1** | **27 (v3.11.0 计划完整)** | **~99%** |

### 6.2 168h SOAK 启动

Post-GA 持续监控已启动 (2026-07-14 13:33 UTC, 预计 2026-07-21 完成):

- 服务器: `sqlrustgo-mysql-server` v3.10.0 GA (commit `8056d5fb66`)
- 工作负载: TPC-H Q1/Q6/Q12/Q14 轮询 + 8 线程 OLTP 自定义
- 当前状态 (5h 37m 后): RSS 1.7GB 稳定, FD 25, CPU 237%, 0 错误

### 6.3 并行执行器优化 (Issue #3792)

6 项优化已落地:
1. PARALLEL_MIN_ROWS=2M
2. 并行触发前置判断 (2x overhead gate)
3. 性能埋点 (partition_ms / filter_ms / merge_ms / total_ms)
4. Batch-Parallel 8K 行 chunks
5. Rayon 线程数动态配置
6. 自适应并行度选择
