...
SQLHENV hEnv;
SQLHDBC hDbc;
SQLCHAR* connStr = "Driver={MySQL ODBC 8.0 Driver};Server=127.0.0.1;Port=3306;Database=default;User=root;";
...
// 执行查询
SQLExecDirect(hDbc, "SELECT * FROM users", SQL_NTS);
```
...
| POST | /api/v1/execute | 执行 DDL/DML |
| GET | /api/v1/tables | 列出表 |
| GET | /api/v1/table/{name} | 表结构 |
