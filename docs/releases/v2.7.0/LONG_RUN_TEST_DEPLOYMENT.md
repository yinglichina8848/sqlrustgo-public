# SQLRustGo v2.7.0 72 小时长稳测试部署与监控指南

> **版本**: v2.7.0 GA
> **发布日期**: 2026-04-22
> **测试类型**: 72 小时长稳测试 (T-10 里程碑)

---

## 一、概述

本文档描述如何在服务器上部署和执行 SQLRustGo v2.7.0 的 72 小时长稳测试，包括环境准备、部署步骤、监控方法和结果收集。
...
# 或者手动执行
bash scripts/test/deploy_stability_test.sh --branch release/v2.7.0 --hours 72
```
...
v2.7.0 的 T-10 里程碑需要满足以下额外要求：

| 验收项 | 要求 | 说明 |
|--------|------|------|
| 72h 稳定性 | 零崩溃 | 测试期间不得有任何崩溃 |
| 内存稳定性 | 无内存泄漏 | 内存使用增长率 < 5%/h |
| QPS 稳定性 | ≥ 1000 QPS | 连续 72h 测试期间 |
| 事务一致性 | ACID 保证 | 所有事务符合 ACID 属性 |
| 并发处理 | 100+ 并发连接 | 稳定处理 100 并发 |
...
1. **72 小时连续运行证明**: 测试日志时间戳
2. **资源使用曲线图**: CPU、内存、网络 72h 变化
3. **QPS 统计**: 平均值、中位数、99 分位
4. **异常事件列表**: 任何异常的时间、类型、处理方式
5. **性能对比**: 与 v2.6.0 相比的性能提升
...
```bash
# 1. 创建当前版本备份
git tag backup/v2.7.0-ga-$(date +%Y%m%d)
