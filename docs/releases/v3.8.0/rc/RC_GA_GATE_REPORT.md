...
## Cross-Version Debt Delta (D6)

> **Spec**: SPEC-008 | **ADR**: ADR-010 | **Auto-generated by**: `bash scripts/gate/check_cross_version_debt.sh`
> **2026-06-04 sync**: Per PR #3097 audit + Issue #3104, INT-1/INT-4 已 CLOSED
> (PR-3019+PR-3050 / PR-2999+PR-3051). Total: 9 ACTIVE / 2 CLOSED / 0 DEFERRED.

| 类别 | ACTIVE | CLOSED | DEFERRED |
|------|--------|--------|----------|
| Integration | 2 (INT-2, INT-3) | 2 (INT-1, INT-4) | 0 |
| Architecture | 3 (ARCH-1/2/3) | 0 | 0 |
| Semantic | 4 (SEM-1/2/3/4) | 0 | 0 |
| **Total** | **9** | **2** | **0** |

**Delta (vs v3.7.0)**: -2 ACTIVE, +2 CLOSED, +0 DEFERRED

**Verdict**: ⚠️ **NEEDS_ATTENTION** — 9 ACTIVE debt, 2 progress this release

**Per ADR-010 Decision-3**:
- ACTIVE 跨 3 个版本 → P0 升级
- ACTIVE 跨 5 个版本 → GA 阻断
- INT-1 跨 7 版本 → 已 CLOSED (GA 阻断消除)
- 剩余跨 5 版本: INT-2 (v2.6.0+), INT-3 (v3.0.0+), ARCH-1, ARCH-2, ARCH-3, SEM-1~4 (大部分 v3.0.0+)

**Action Item**: 9 ACTIVE debt 需在 v3.8.0+1 / v3.9.0 制定修复计划（详见 `docs/releases/v3.8.0/CROSS-VERSION-DEBT.md`）
