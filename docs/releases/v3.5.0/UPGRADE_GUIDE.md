...
```bash
sqlrustgo backup create /backup/pre-v350-$(date +%Y%m%d)
```
...
# 4. 替换二进制
sudo -S -p '' cp sqlrustgo-v3.5.0/sqlrustgo /usr/local/bin/
...
sed -i 's/v3.4.0/v3.5.0/g' docker-compose.yaml
...
```bash
# 启动 GMP 服务器
sqlrustgo serve --gmp-enabled

# 测试报告生成
curl -X POST http://localhost:8080/api/v1/gmp/report/translate \
  -H "Content-Type: application/json" \
  -d '{"text": "批生产记录", "target_lang": "en"}'
```
...
```bash
# 测试偏差调查
curl -X POST http://localhost:8080/api/v1/gmp/investigate \
  -H "Content-Type: application/json" \
  -d '{"deviation_id": "DEV-001"}'
```
...
# 2. 恢复备份
sqlrustgo backup restore /backup/pre-v350-20260529
...
A: v3.5.0 默认启用 AI 功能，会有 5-10% 性能开销。可禁用：
```toml
[gmp]
enabled = false
```
...
```toml
[gmp]
mode = "local"  # 本地模式，无 LLM 开销
```

### Q: 配置文件不兼容？

A: v3.5.0 完全向后兼容 v3.4.0 配置。新增配置项有默认值，可安全添加。
