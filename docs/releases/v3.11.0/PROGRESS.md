...
> **说明**: 本中文主文用于当前审阅；英文原文保留在附录。若本文件中的进度与 Gitea issue、PR、commit 或综合评估冲突，以最新可验证证据为准。

## 1. 报告定位

本文件用于记录 v3.11.0 工作进度，不作为 gate PASS 的直接证据。进度项必须通过 PR merge、commit、测试输出或 release report 复核。

## 2. 当前阅读规则

- 已完成项需要有对应 PR/commit 或测试输出。
- 未完成、部分完成、延期项应进入 v3.12 issue plan。
- 任何“完成/通过”表述都需要 evidence hash。
- 历史进度中的英文状态词如 DONE、MERGED、IN_PROGRESS 保留为状态标签，但中文解释优先。

## 3. 重点后续

v3.12 应继续跟踪 SQLLogicTest、TPC-H correctness、coverage 口径统一、wire protocol、LOAD DATA、crash recovery、backup/restore、upgrade/downgrade，以及 GMP 内审检索生产化。
...
> **Version**: v3.11.0
> **Updated**: 2026-08-08
> **Status**: RC (GA blocked)
> **SSOT note**: Stage state is governed by `STAGE.yaml`; GA readiness is governed by `GA_GATE_REPORT.md`.
...
| TODO | 10 (43.5%) |
| Total Effort | ~1156h |
| GA Target | BLOCKED_UNTIL_GA_GATE_PASS |
...
| Task | Status | Notes |
|------|--------|-------|
| V311-20 (TPC-H SF=1) | GA BLOCKED | fixture + real 22/22 evidence required |
...
| Query | Status | Notes |
|-------|--------|-------|
| Q1-Q22 | GA BLOCKED | Real 22/22 execution not evidenced; see `TPCH_SF1_VERIFICATION_REPORT.md` |
...
| Coverage (RC) | ≥75% | TBD | ⏳ |
| Coverage (GA) | ≥80% | TBD | ⏳ |
| TPC-H SF=1 | 22/22 required | real 22/22 evidence missing | ❌ |
...
| 2026-08-08 | Codex | Corrected current stage to RC / GA blocked and removed SF=1 22/22 PASS claim |

---

*This document tracks task progress. For release stage and GA readiness, use `STAGE.yaml` and `GA_GATE_REPORT.md`.*
