...
| 原则 | 说明 | 违规处理 |
|------|------|----------|
| **计划不可伪造** | 开发计划、测试计划是历史记录，禁止为通过门禁而重写原始计划 | 立即回退并问责 |
| **原始记录保留** | 计划文档只追加、不改写；状态变更通过版本对比而非文档改写 | 永久保留原始版本 |
| **执行结果优先** | 以实际执行结果（命令输出、测试通过）为依据，禁止编造测试报告 | 以证据为准 |
| **禁止形式主义** | 门禁是质量关卡，不是文档美化关卡 | Truthfulness 零容忍 |

**违规定义**：
- 禁止将计划文档重写为"最终报告"以掩盖未完成的工作
- 禁止修改开发/测试计划状态（Alpha/Beta/RC/GA）以通过门禁
- 禁止编造测试结果或测试报告
- 禁止仅修改文档状态而不执行实际验证

**正确做法**：
- 计划变更是正常的，用新文档追加，不改写原始计划
- 状态变更是执行结果，在门禁检查后记录，不预先修改文档
- 测试结果是命令输出的记录，不是文档编造的内容
...
**常见引用问题**:
- 引用不存在的文件: `[UPGRADE_GUIDE_v2.7.0.md](../releases/v3.7.0/UPGRADE_GUIDE.md)`
- 引用路径错误: `[文档](../releases/v3.7.0/UPGRADE_GUIDE.md)` 而非 `[文档](../releases/v3.7.0/UPGRADE_GUIDE.md)`
