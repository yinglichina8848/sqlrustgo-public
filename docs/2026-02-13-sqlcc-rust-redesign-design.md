...
| 工具类别 | 工具 | 教学定位 | 使用阶段 |
|:---------|:-----|:--------:|:--------:|
| **多Agent协作** |开爪| 主多Agent框架，执行复杂任务 | 全流程 |
| **规范控制** |开放规范|SDD/TDD 规范控制 CLI| 设计/测试 |
| **Agent团队** |代理团队|Claude Code 多Agent并行| 开发/审查 |
| **AI编程助手** |克劳德·科德| 主AI工具，全流程使用 | 全程 |
| **代码补全** |GitHub 副驾驶| 实时补全，语法提示 | 编码阶段 |
| **AI审查** |码兔AI| 自动化代码审查 | PR阶段 |
| **IDE** |VS 代码/Zed| 主开发环境 | 全程 |
| **CLI工具** |uv / ripgrep / fd| 高效命令行操作 | 全程 |
| **CI/CD** |GitHub 操作| 自动化流水线 | 集成阶段 |
...
| 工具 | 模式 | 命令示例 | 使用场景 |
|:-----|:-----|:--------|:---------|
|**张开爪**| 多Agent执行 |__代码0__| 执行复杂多步骤任务 |
|**开放规格**| 规范验证 |__代码0__| 验证设计规范 |
|**代理团队**| 并行开发 |__代码0__| 多任务并行处理 |
|**克劳德·代码**| 任务执行 |__代码0__| 单一任务执行 |
...
#### SQL 解析器：ANTLR4 或 rust-arctic

| 方案 | 优点 | 缺点 |
|------|------|------|
| ANTLR4 | 成熟的语法树生成，SQL 语法规则丰富 | 需要安装 Java 运行时 |
|铁锈北极| 纯 Rust 实现，无外部依赖 | 语法规则较少 |
...
|------|------|
| gRPC | 内部服务通信、云原生接口 |
|MySQL 连线| 客户端兼容性（mysql-client 可直接连接） |
...
| 周次 |开爪|开放规范|代理团队|克劳德·科德|副驾驶| CLI | 其他 |
|:----:|:---------:|:--------:|:----------:|:-----------:|:-------:|:---:|:----:|
...
| 7-8 | 优化分析 | 性能规范 | 进度追踪 | 优化分析 | 优化补全 |火焰图| 基准测试 |
| 9-10 | 重构任务 | 重构规范 | 结果聚合 | 重构建议 | 重构补全 |懒惰的吉特|复杂度分析 |
| 11-12 | 事务开发 | ACID规范 | 并发协调 | 事务生成 | 并发补全 | tmux | 压力测试 |
| 13-14 | 监控开发 | 监控规范 | 告警配置 | 监控生成 | 日志补全 |库贝克特尔|格拉法纳|
| 15-16 | 部署任务 | 发布规范 | 交付确认 | 部署脚本 | 文档补全 |泊坞窗| K9s |
...
| 第6周 | 04-06~04-12 | 缓冲池 | 第6章3-4节 |缓冲池|
...
| 周次 | 日期 | 主题 | 教材内容 | 实践任务 |
|:----:|:----:|:-----|:--------:|:--------:|
| 第7周 | 04-13~04-19 |分析| 第7章1-2节 |性能/火焰图|
...
| **功能完成** | 20% | 实现功能完整性 |
| **多Agent协同** | 15% |OpenClaw(5%) + OpenSpec(5%) + 代理团队(5%)|
| **AI工具使用** | 15% |克劳德·代码(8%) + 副驾驶(4%) + CLI(3%)|
...
|:-----|:-----|:-----|
| `/task` | 执行明确任务 |__代码0__|
|__代码0__| TDD 模式 |__代码0__|
|__代码0__| 代码审查 |__代码0__|
|__代码0__| 重构建议 | `/refactor 简化代码` |
|__代码0__| 解释代码 | `/explain 这段代码逻辑` |
| `/debug` | 调试辅助 |__代码0__|
...
| 第2周 | 02-22 ~ 02-28 |SQL解析器（DDL+DML）| 语法解析器 |
...
1.《事务处理：概念和技术》——Jim Gray
2.“数据库系统概念”-Silberschatz、Korth、Sudarshan
3.“设计数据密集型应用程序”——Martin Kleppmann
4.“火山迭代器模型”——Goetz Graefe
