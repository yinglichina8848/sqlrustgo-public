...
fn diag_q11_steps() {
    ...
    let r1 = engine
        .execute("SELECT COUNT(*) FROM partsupp INNER JOIN supplier ON ps_suppkey = s_suppkey")
        .unwrap();
    eprintln!(
        "Step1 1-join: rows.len()={}, first={:?}",
        r1.rows.len(),
        r1.rows.first()
    );
    ...
    eprintln!(
        "Step3 3-way comma: rows.len()={}, first={:?}",
        r3.rows.len(),
        r3.rows.first()
    );
