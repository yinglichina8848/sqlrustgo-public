...
fn test_prepare_execute_basic() {
    ...
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 100)").unwrap();
    engine.execute("PREPARE stmt1 AS 'SELECT * FROM t WHERE id = 1'").unwrap();
...
fn test_prepare_overwrite() {
    ...
    engine.execute("PREPARE s1 AS 'SELECT * FROM t WHERE id = 1'").unwrap();
    let r1 = engine.execute("EXECUTE s1").unwrap();
    assert_eq!(r1.rows.len(), 1);
    engine.execute("PREPARE s1 AS 'SELECT * FROM t WHERE id = 2'").unwrap();
...
fn test_prepare_select_with_where() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, age INTEGER)").unwrap();
    engine.execute("INSERT INTO users VALUES (1, 20), (2, 30), (3, 40)").unwrap();
    engine.execute("PREPARE adults AS 'SELECT * FROM users WHERE age >= 30'").unwrap();
...
fn test_prepare_with_insert() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE log (id INTEGER PRIMARY KEY, msg TEXT)").unwrap();
    engine.execute("PREPARE ins AS 'INSERT INTO log VALUES (1, ''hello'')'").unwrap();
...
fn test_prepare_with_update() {
    ...
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 100)").unwrap();
    engine.execute("PREPARE upd AS 'UPDATE t SET v = 200 WHERE id = 1'").unwrap();
...
fn test_prepare_with_aggregate() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)").unwrap();
    engine.execute("PREPARE sumq AS 'SELECT SUM(v) FROM t'").unwrap();
...
fn test_prepare_with_join() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE a (id INTEGER PRIMARY KEY, x INTEGER)").unwrap();
    engine.execute("CREATE TABLE b (a_id INTEGER, y TEXT)").unwrap();
    engine.execute("INSERT INTO a VALUES (1, 10), (2, 20)").unwrap();
    engine.execute("INSERT INTO b VALUES (1, 'one'), (2, 'two')").unwrap();
    engine.execute("PREPARE j AS 'SELECT a.x, b.y FROM a JOIN b ON a.id = b.a_id'").unwrap();
...
fn test_prepare_with_subquery() {
    ...
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 10), (2, 20)").unwrap();
    engine.execute("PREPARE sub AS 'SELECT * FROM (SELECT v FROM t WHERE v > 15) AS s'").unwrap();
...
fn test_multiple_prepared_statements() {
    ...
    engine.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)").unwrap();
    engine.execute("PREPARE s1 AS 'SELECT * FROM t WHERE id = 1'").unwrap();
    engine.execute("PREPARE s2 AS 'SELECT * FROM t WHERE id = 2'").unwrap();
    engine.execute("PREPARE s3 AS 'SELECT * FROM t WHERE id = 3'").unwrap();
    let r1 = engine.execute("EXECUTE s1").unwrap();
...
fn test_prepare_with_create_table() {
    let mut engine = make_engine();
    engine.execute("PREPARE c AS 'CREATE TABLE t (id INTEGER PRIMARY KEY, v TEXT)'").unwrap();
...
fn test_prepare_with_text() {
    ...
    engine.execute("INSERT INTO t VALUES (1, 'hello world')").unwrap();
    engine.execute("PREPARE q AS 'SELECT name FROM t WHERE id = 1'").unwrap();
...
fn test_prepare_with_null() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v TEXT)").unwrap();
...
fn test_prepare_with_like() {
    ...
    engine.execute("INSERT INTO t VALUES (2, 'banana')").unwrap();
    engine.execute("PREPARE q AS 'SELECT name FROM t WHERE name LIKE ''a%'''").unwrap();
    let r = engine.execute("EXECUTE q").unwrap();
    assert_eq!(r.rows.len(), 1);
}
...
fn test_prepare_with_group_by() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, grp INTEGER, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 1, 10), (2, 1, 20), (3, 2, 30)").unwrap();
    engine.execute("PREPARE q AS 'SELECT grp, SUM(v) FROM t GROUP BY grp ORDER BY grp'").unwrap();
    let r = engine.execute("EXECUTE q").unwrap();
    assert_eq!(r.rows.len(), 2);
...
fn test_prepare_with_order_by() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 30), (2, 10), (3, 20)").unwrap();
    engine.execute("PREPARE q AS 'SELECT v FROM t ORDER BY v ASC'").unwrap();
    let r = engine.execute("EXECUTE q").unwrap();
...
fn test_prepare_with_limit() {
    ...
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    for i in 0..100 {
        engine.execute(&format!("INSERT INTO t VALUES ({}, {})", i, i)).unwrap();
    }
    engine.execute("PREPARE q AS 'SELECT v FROM t ORDER BY v LIMIT 10'").unwrap();
    let r = engine.execute("EXECUTE q").unwrap();
...
fn test_prepare_with_distinct() {
    let mut engine = make_engine();
    engine.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)").unwrap();
    engine.execute("INSERT INTO t VALUES (1, 10), (2, 10), (3, 20)").unwrap();
    engine.execute("PREPARE q AS 'SELECT DISTINCT v FROM t ORDER BY v'").unwrap();
    let r = engine.execute("EXECUTE q").unwrap();
    assert_eq!(r.rows.len(), 2);
}
...
fn test_perf_execute_smoke() {
    ...
    engine.execute("PREPARE q AS 'SELECT COUNT(*) FROM t'").unwrap();
    let r1 = engine.execute("EXECUTE q").unwrap();
    assert_eq!(r1.rows.len(), 1);
    let r2 = engine.execute("EXECUTE q").unwrap();
    assert_eq!(r2.rows.len(), 1);
