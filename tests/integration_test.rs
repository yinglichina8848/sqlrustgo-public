...
fn test_full_select_flow() {
    ...
    // Create table first
    engine.execute(parse("CREATE TABLE users").unwrap()).unwrap();
...
fn test_multiple_statements() {
    ...
    engine.execute(parse("CREATE TABLE t1 (id INT)").unwrap()).unwrap();
    engine.execute(parse("CREATE TABLE t2 (id INT)").unwrap()).unwrap();
