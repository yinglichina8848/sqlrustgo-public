//! Q11 isolated test - to see the actual debug output for aggregate lookup
...
fn diag_q11_3way() {
    ...
    // Q11 full
    let r = engine.execute("SELECT ps_partkey, SUM(ps_supplycost * ps_availqty) AS part_value FROM partsupp, supplier, nation WHERE ps_suppkey = s_suppkey AND s_nationkey = n_nationkey AND n_name = 'GERMANY' GROUP BY ps_partkey HAVING SUM(ps_supplycost * ps_availqty) > 10000 ORDER BY part_value DESC").unwrap();
    eprintln!("Q11 full: rows.len() = {}, expected 80", r.rows.len());
