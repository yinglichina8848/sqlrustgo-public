...
fn three_table_join_preserves_all_tables() {
    ...
    let mut e = engine();
    e.execute("CREATE TABLE customer (c_custkey INTEGER, c_name TEXT)")
        .unwrap();
    e.execute("CREATE TABLE orders (o_orderkey INTEGER, o_custkey INTEGER)")
        .unwrap();
    e.execute("CREATE TABLE lineitem (l_orderkey INTEGER, l_qty INTEGER)")
        .unwrap();
    e.execute("INSERT INTO customer VALUES (1, 'Alice')")
        .unwrap();
    e.execute("INSERT INTO customer VALUES (2, 'Bob')").unwrap();
    e.execute("INSERT INTO customer VALUES (3, 'Carol')")
        .unwrap();
