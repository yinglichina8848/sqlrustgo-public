...
fn parse_create_function_with_begin_end_no_as() {
    ...
    assert!(parse(sql).is_ok(), "BEGIN/END without AS must parse: {:?}",
            parse(sql).err());
}
...
fn parse_create_function_with_begin_end_with_as() {
    let sql = "CREATE FUNCTION f2(x INT) RETURNS INT AS \
               BEGIN DECLARE r INT; SET r = x + 100; RETURN r; END";
    assert!(parse(sql).is_ok(), "AS BEGIN/END must still parse: {:?}",
            parse(sql).err());
}
...
fn parse_create_function_single_expr_still_works() {
    ...
    assert!(parse(sql).is_ok(), "single-expression RETURN body must still parse");
}
...
fn parse_create_function_with_returns_table() {
    ...
    assert!(parse(sql).is_ok(), "RETURNS TABLE(...) must parse: {:?}",
            parse(sql).err());
}
...
fn parse_create_function_with_returns_table_and_begin_end() {
    ...
    assert!(parse(sql).is_ok(), "RETURNS TABLE with BEGIN/END must parse: {:?}",
            parse(sql).err());
}