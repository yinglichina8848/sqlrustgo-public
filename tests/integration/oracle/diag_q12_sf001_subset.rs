//! V312-58 / Issue #4378 — Q12 SF~0.001 subset reproduction.
//!
//! Bug: TPC-H Q12 (shipping-modes-order-priority) returns 7 rows instead of 2.
//! SQLite oracle for SF~0.001 subset (2 rows):
//!   MAIL|60|101
//!   SHIP|54|99
//!
//! Pre-fix expected (per issue #4378): 7 rows — extra shipmodes participate.
//!
//! Run:
...
fn q12_sf001_subset_count() {
    let mut engine = setup();
    let r = engine
        .execute(Q12_SQL)
        .unwrap_or_else(|e| panic!("Q12 failed: {}", e));
    eprintln!(
        "Q12 SF~0.001 subset returned {} rows × {} cols",
        r.rows.len(),
