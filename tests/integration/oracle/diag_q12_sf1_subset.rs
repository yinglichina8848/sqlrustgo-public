//! V312-58 / Issue #4378 — Q12 SF~0.1 subset reproduction.
//!
//! Subset: 150K orders (head of SF=1 orders), 600K lineitem (matching l_orderkey).
//!
//! SQLite ground truth for SF~0.1 subset (2 rows):
//!   MAIL|647|945
//!   SHIP|620|943
//!
...
const DATA_DIR: &str = "/tmp/tpch_sf1_subset";
...
fn q12_sf1_subset_count() {
    let mut engine = setup();
    let r = engine
        .execute(Q12_SQL)
        .unwrap_or_else(|e| panic!("Q12 failed: {}", e));
    eprintln!(
        "Q12 SF~0.1 subset returned {} rows × {} cols",
        r.rows.len(),
    ...
    eprintln!("  MAIL|647|945");
    eprintln!("  SHIP|620|943");
}
