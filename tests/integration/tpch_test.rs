...
mod tests {
    ...
    fn setup_tpch_data() -> ExecutionEngine<MemoryStorage> {
    ...
    fn test_between_operator() {
        ...
        let result = engine
            .execute("SELECT * FROM t WHERE a BETWEEN 1 AND 10")
            .unwrap();
        assert_eq!(result.rows.len(), 2);

        let result = engine
            .execute("SELECT * FROM t WHERE a BETWEEN 6 AND 20")
            .unwrap();
        assert_eq!(result.rows.len(), 2);
    }
    ...
    fn test_in_list_operator() {
        ...
        let result = engine
            .execute("SELECT * FROM t WHERE a IN ('MAIL', 'SHIP')")
            .unwrap();
        assert_eq!(result.rows.len(), 2);
    }
    ...
    fn test_in_list_operator_three_values() {
        ...
        let result = engine
            .execute("SELECT * FROM t WHERE a IN ('MAIL', 'AIR', 'TRUCK')")
            .unwrap();
