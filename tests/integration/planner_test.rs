...
fn test_explain_physical_plan() {
    ...
    let explain_plan = LogicalPlan::Explain {
        input: Box::new(table_scan),
        analyze: false,
    };

    let planner = DefaultPlanner::new();
    let result = planner.create_physical_plan(&explain_plan);

    assert!(result.is_ok());
    let physical_plan = result.unwrap();
    assert_eq!(physical_plan.name(), "Explain");
...
#[test]
fn test_explain_analyze_flag() {
    use sqlrustgo_planner::ExplainExec;
    ...
    let explain_exec = physical_plan
        .as_any()
        .downcast_ref::<ExplainExec>()
        .expect("Should be ExplainExec");
    assert!(explain_exec.analyze());
}
