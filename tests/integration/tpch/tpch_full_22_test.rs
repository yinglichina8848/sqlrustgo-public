//! TPC-H Full 22 Query Test — wire protocol (LOAD DATA LOCAL INFILE + MySqlTestClient)
//!
//! Migrated from in-process ExecutionEngine to wire protocol per
//! docs/plans/2026-06-13-tpch-e2e-migration-design.md §3.2.
//!
//! Default SF: 0.001 (uses checked-in fixture at tests/data/tpch-sf001/).
//!
...
fn tpch_full_22_queries() {
    ...
    eprintln!("\n=== TPC-H Full 22: {passed}/{total} passed in {total_elapsed:.2?} ===");
    if !failed.is_empty() {
        panic!("{}/{} queries failed: {:#?}", failed.len(), total, failed);
    }
}
