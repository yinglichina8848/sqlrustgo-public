...
fn test_text_index() {
    ...
    engine
        .execute(parse("CREATE INDEX idx_status ON orders(o_orderstatus)").unwrap())
        .unwrap();
        ...
        match storage.bulk_load_tbl_file("lineitem", &filepath) {
            Ok(count) => {
                ...
                match engine.execute(
                    parse("SELECT COUNT(*) FROM lineitem WHERE l_returnflag = 'R'").unwrap(),
                ) {
                    Ok(result) => {
                        let elapsed = start.elapsed();
                        println!("  Result: {:?} in {:?}", result.rows, elapsed);
                ...
                let start = std::time::Instant::now();
                match engine.execute(
                    parse("SELECT COUNT(*) FROM lineitem WHERE l_shipdate <= '1998-09-02'")
                        .unwrap(),
                ) {
                    Ok(result) => {
                        let elapsed = start.elapsed();
